...
class Ball{
    ...
    constructor(scene, camera, board, socket, roomId, allowTrajectory) {
        ...
        this._ballMesh = null; // Stocker le mesh de la balle
        this._impulseForce = 14;
        this._ballWeight = 1.1; // Poids de la balle pour la physique

        this._asBeenThrown = false; // Indicateur si le ball a été lancé
    ...
    resetLocation(){
        this._ballMesh.parent.position = this.location; // Réinitialiser la position du dé
      }
    ...
    async loadModel(newLocation = this.location) {
        ...
        try {
            ...
            this._ballMesh = BABYLON.MeshBuilder.CreateSphere("ball", { diameter: 5 }, this._scene);

            const hitbox = BABYLON.MeshBuilder.CreateSphere("ballHitbox", { diameter: 5 }, this._scene);
            hitbox.visibility = 0.4;
            hitbox.isVisible = true;
            hitbox.position = newLocation.clone();

            this._ballMesh.parent = hitbox;
            this._ballMesh.isPickable = true;
            this._ballMesh.doNotSyncBoundingInfo = true;
    ...
    setMeshLocation(arr /* [x,y,z] */) {
            ...
            this._ballMesh.parent.position.copyFromFloats(arr[0], arr[1], arr[2]);
          }
        
          setMeshRotation(arr /* [x,y,z,w] */) {
            if (!this._ballMesh) return;
            this._ballMesh.parent.rotationQuaternion = BABYLON.Quaternion.FromArray(arr);
          }
    ...
    createTrajectory(startPosition) {
        ...
        const mass = this._ballWeight; // Masse de l'objet (en kg)
    
        // Calculer la direction initiale en fonction de la rotation actuelle de la balle
        const forward = new BABYLON.Vector3(0, 0, 1); // Direction de base (avant)
        const worldMatrix = this._ballMesh.parent.getWorldMatrix();
    ...
    //     this._ballMesh.parent.rotation.x += BABYLON.Tools.ToRadians(angle);
    // }
    ...
    //     this._ballMesh.parent.rotation.y += BABYLON.Tools.ToRadians(angle);
    // }


    adjustPitch(angleDegrees) {
            if (!this._ballMesh || this._asBeenThrown) return;
            const hitbox = this._ballMesh.parent;
    ...
    updateTrajectory() {
        ...
        const startPosition = this._ballMesh.parent.position;
        this.createTrajectory(startPosition); // Recalculer la trajectoire
    }

    removeFromBoard() {
        this._ballMesh.parent.dispose(); // Supprimer le hitbox
        // this._ballMesh.dispose(); // Supprimer le ball
    }
    ...
    providePhysicConsistency() {
        ...
        const hitbox = this._ballMesh.parent;
        if (!hitbox) return;
        const hitboxAggregate = new BABYLON.PhysicsAggregate(hitbox, BABYLON.PhysicsShapeType.SPHERE, { mass: this._ballWeight, friction: 4, restitution: 0 }, this._scene);
        hitboxAggregate.body.setMotionType(BABYLON.PhysicsMotionType.STATIC);
    }


    applyThrowImpulse(onballThrown, onballLanded) {

        if (this._asBeenThrown) {
            console.error("ball has already been thrown.");
            return;
        }

        if (!this._ballMesh) {
            console.error("ball mesh is not loaded.");
            return;
        }
        ...
        // Calculer la direction de l'impulsion en fonction de la rotation actuelle de la balle
        const forward = new BABYLON.Vector3(0, 0, 1); // Direction de base (avant)
        ...
        const forwardSync = () => {
            if (this._ballMesh && !this._ballMesh.isDisposed()) {
              const pos = this._ballMesh.parent.position.asArray();
              const rotQ = this._ballMesh.parent.rotationQuaternion.asArray();
              this._socket.emit("diceFlightUpdate", {
                roomId: this._roomId,
                position: pos,
                rotationQuaternion: rotQ,
        ...
        this._asBeenThrown = true; // Indiquer que le ball a été lancé
        ...
        // Vérifier régulièrement si le ball est immobile
        const interval = setInterval(() => {

            // Si le ball a été dispose, arrêter l'intervalle
            if (this._ballMesh.isDisposed() || hitbox.isDisposed() || !hitboxAggregate || !hitboxAggregate.body) {
                clearInterval(interval); // Arrêter l'intervalle
                console.log("La balle a été supprimée. Arrêt de la vérification.");
                return;
            }

            const linearVelocity = hitboxAggregate.body.getLinearVelocity();
            const isStopped = linearVelocity.length() < 0.05;
            ...
            if (isStopped) {
                    ...
                    row.socles.forEach((socle, socleIndex) => {
                        if (hitbox.intersectsMesh(socle, false)) {
                            ...
                            if (!isNaN(socleIndex) && socleIndex >= 4 && socleIndex <= 7) {
                                ...
                                this._socket.emit("validateThrow", { roomId: this._roomId, item : "ball", color: this._color, row: rowIndex, col: socleIndex, value: this._value }, (response) => {
                                    if (response.success) {
                                        console.log("Matrice mise à jour sur le serveur.");
                                        this._ballMesh.dispose(); // Supprimer le mesh du dé
                                        hitbox.dispose(); // Supprimer le hitbox
                                    }
                                });

                            } else {
                                console.log("balle invalide car elle n'est pas dans la bonne section.");
                                this._ballMesh.dispose(); // Supprimer le mesh de la balle
                                hitbox.dispose(); // Supprimer le hitbox
                                return;
                            }
                        }
                ...
                if (!isValid) {
                    console.log("balle invalide car elle n'est arrivée sur aucun socle.");
                    this._ballMesh.dispose(); // Supprimer le mesh de la ball
                    hitbox.dispose(); // Supprimer le hitbox
                    // if (onDiceLanded) {
                    //     onDiceLanded(false);
                    // }
                    this._socket.emit("invalidateThrow", { roomId: this._roomId, item : "ball", color : this._color  }, (response) => {
                        if (response.success) {
        ...
        // Vérifier si la balle entre en collision avec le sol dans la boucle de rendu
        const ground = this._scene.getMeshByName("ground"); // Assurez-vous que le sol est nommé "ground"
        const checkCollision = () => {
            if (this._ballMesh.isDisposed() || hitbox.isDisposed()) {
                this._scene.onBeforeRenderObservable.remove(checkCollision); // Arrêter la vérification
                return;
            }

            // Vérifier si la balle entre en collision avec le sol
            if (ground && hitbox.intersectsMesh(ground, false)) {
                console.log("Le ball est entré en contact avec le sol. Suppression...");
                this._ballMesh.dispose(); // Supprimer le mesh de la balle
                hitbox.dispose(); // Supprimer le hitbox
                this._scene.onBeforeRenderObservable.remove(checkCollision); // Arrêter la vérification
                // if (onDiceLanded) {
    ...
    drop(rowIndex, socleIndex) {
            ...
            const interval = setInterval(() => {
                ...
                if (isStopped) {
                    ...
                    this._socket.emit("ballDrop", { roomId: this._roomId, color: this._color, row: rowIndex, col: socleIndex, value: this._value }, (response) => {
                        if (response.success) {
                            console.log("Matrice mise à jour sur le serveur.");
                        }
                    });
