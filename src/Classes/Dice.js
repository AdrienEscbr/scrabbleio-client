...
import WHITE_DICE_URL from "../Assets/Models/WhiteDice.glb";
import RED_DICE_URL from "../Assets/Models/RedDice.glb";

class Dice {
  constructor(color, scene, camera, board, socket, roomId, allowTrajectory) {
    this._value = 0;
    this._color = color;
    this._scene = scene;
    this._camera = camera;
    this._diceFaces = {
      3: new BABYLON.Vector3(0, -1, 0), // Top face
      5: new BABYLON.Vector3(0, 1, 0), // Bottom face
      6: new BABYLON.Vector3(-1, 0, 0), // Right face
      1: new BABYLON.Vector3(1, 0, 0), // Left face
      2: new BABYLON.Vector3(0, 0, -1), // Front face
      4: new BABYLON.Vector3(0, 0, 1), // Back face
    };

    this._trajectory = null; // Stocker la trajectoire
    this._trajectorySpheres = [];
    this._diceMesh = null; // Stocker le mesh du dé
    this._impulseForce = 14;
    this._diceWeight = 1.1; // Poids du dé pour la physique

    this._asBeenThrown = false; // Indicateur si le dé a été lancé
  ...
  resetLocation(){
    this._diceMesh.parent.position = this.location; // Réinitialiser la position du dé
  }
  ...
  async loadModel(newLocation = this.location) {
    ...
    try {
      const url = this._color === "white" ? WHITE_DICE_URL : RED_DICE_URL;
      ...
      this._diceMesh = result.meshes[0];
      ...
      this._diceMesh.parent = hitbox;
      this._diceMesh.isPickable = true;
      this._diceMesh.doNotSyncBoundingInfo = true;
      ...
      const highligther = new BABYLON.HighlightLayer(
        "highlighter",
        this._scene
      );
  ...
  //     this._diceMesh.parent.position = location; // Cloner la position pour éviter les modifications accidentelles
  // }

  // getMeshLocation() {
  //     if (!this._diceMesh) return null;
  //     return this._diceMesh.parent.position; // Cloner la position pour éviter les modifications accidentelles
  // }

  setMeshLocation(arr /* [x,y,z] */) {
    if (!this._diceMesh) return;
    // position est un Vector3 : on lève ses composantes
    this._diceMesh.parent.position.copyFromFloats(arr[0], arr[1], arr[2]);
  }

  setMeshRotation(arr /* [x,y,z,w] */) {
    if (!this._diceMesh) return;
    this._diceMesh.parent.rotationQuaternion =
      BABYLON.Quaternion.FromArray(arr);
  }
  ...
  createTrajectory(startPosition) {
    ...
    const mass = this._diceWeight; // Masse de l'objet (en kg)

    // Calculer la direction initiale en fonction de la rotation actuelle du dé
    const forward = new BABYLON.Vector3(0, 0, 1); // Direction de base (avant)
    const worldMatrix = this._diceMesh.parent.getWorldMatrix();
    const direction = BABYLON.Vector3.TransformNormal(
      forward,
      worldMatrix
    ).normalize(); // Utiliser TransformNormal pour inclure la rotation
    ...
    this._trajectory = BABYLON.MeshBuilder.CreateLines(
      "trajectory",
      { points },
      this._scene
    );
  ...
  adjustPitch(angleDegrees) {
    if (!this._diceMesh || this._asBeenThrown) return;
    const hitbox = this._diceMesh.parent;
    if (!hitbox.rotationQuaternion) {
      // Initialiser la quaternion si ce n’est pas déjà fait
      hitbox.rotationQuaternion = BABYLON.Quaternion.FromEulerVector(
        hitbox.rotation
      );
    }
  ...
  updateTrajectory() {
    ...
    const startPosition = this._diceMesh.parent.position;
    this.createTrajectory(startPosition); // Recalculer la trajectoire
  }

  logVisibleFace() {
    if (!this._diceMesh) {
      console.error("Dice mesh is not loaded.");
      return "0";
    }
    ...
    // Parcourir les normales des faces pour trouver la face visible
    for (const [faceValue, normal] of Object.entries(this._diceFaces)) {
      const worldNormal = BABYLON.Vector3.TransformNormal(
        normal,
        worldMatrix
      ).normalize();
      const dot = BABYLON.Vector3.Dot(worldNormal, BABYLON.Vector3.Up());
    ...
    console.log(`Face visible : ${visibleFace}`);
    return visibleFace;
  }

  removeFromBoard() {
    // this._diceMesh.dispose(); // Supprimer le mesh du dé
    this._diceMesh.parent.dispose(); // Supprimer le hitbox
  }
  ...
  providePhysicConsistency() {
    ...
    const hitbox = this._diceMesh.parent;
    if (!hitbox) return;
    const hitboxAggregate = new BABYLON.PhysicsAggregate(
      hitbox,
      BABYLON.PhysicsShapeType.BOX,
      { mass: this._diceWeight, friction: 4, restitution: 0 },
      this._scene
    );
  ...
  applyThrowImpulse(onDiceThrown, onDiceLanded) {
    if (this._asBeenThrown) {
      console.error("Dice has already been thrown.");
      return;
    }
    ...
    // Calculer la direction de l'impulsion en fonction de la rotation actuelle du dé
    const forward = new BABYLON.Vector3(0, 0, 1); // Direction de base (avant)
    ...
    const forwardSync = () => {
      if (this._diceMesh && !this._diceMesh.isDisposed()) {
        const pos = this._diceMesh.parent.position.asArray();
        const rotQ = this._diceMesh.parent.rotationQuaternion.asArray();
        this._socket.emit("diceFlightUpdate", {
          roomId: this._roomId,
          position: pos,
          rotationQuaternion: rotQ,
    ...
    // // stocke la fonction pour plus tard
    // this._onFlightUpdate = forwardSync;
    // // et l'ajoute à la boucle
    // this._scene.onBeforeRenderObservable.add(forwardSync);
    // ajoute à la boucle et récupère l'observer
    this._flightObserver = this._scene.onBeforeRenderObservable.add(forwardSync);


    this._asBeenThrown = true; // Indiquer que le dé a été lancé
    ...
    // Vérifier régulièrement si le dé est immobile
    const interval = setInterval(() => {
      // Si le dé a été dispose, arrêter l'intervalle
      if (
        this._diceMesh.isDisposed() ||
        hitbox.isDisposed() ||
        !hitboxAggregate ||
        !hitboxAggregate.body
      ) {
        clearInterval(interval); // Arrêter l'intervalle
        console.log("Le dé a été supprimé. Arrêt de la vérification.");
        return;
      }
      ...
      if (isStopped) {
        ...
        // this._scene.onBeforeRenderObservable.remove(this._onFlightUpdate);
        ...
        this._board.rows.forEach((row, rowIndex) => {
          row.socles.forEach((socle, socleIndex) => {
            if (hitbox.intersectsMesh(socle, false)) {
              ...
              if (!isNaN(socleIndex) && socleIndex >= 0 && socleIndex <= 3) {
                console.log("Dé valide");
                isValid = true;
                let diceValue = parseInt(this.logVisibleFace()); // Afficher la face visible
                // if (onDiceLanded) {
                //     onDiceLanded(false);
                // }
                this.finalRowSlot = [rowIndex, socleIndex]; // Stocker la position finale du dé
                this._socket.emit(
                  "validateThrow",
                  {
                    roomId: this._roomId,
                ...
                console.log(
                  "Dé invalide car il n'est pas dans la bonne section."
                );
                this._diceMesh.dispose(); // Supprimer le mesh du dé
        ...
        if (!isValid) {
          console.log("Dé invalide car il n'est arrivé sur aucun socle.");
          this._diceMesh.dispose(); // Supprimer le mesh du dé
          hitbox.dispose(); // Supprimer le hitbox
          // if (onDiceLanded) {
          //     onDiceLanded(false);
          ...
          this._socket.emit(
            "invalidateThrow",
            { roomId: this._roomId, item: "dice", color: this._color },
            (response) => {
              if (response.success) {
    ...
    // Vérifier si le dé entre en collision avec le sol dans la boucle de rendu
    const ground = this._scene.getMeshByName("ground"); // Assurez-vous que le sol est nommé "ground"
    const checkCollision = () => {
      if (this._diceMesh.isDisposed() || hitbox.isDisposed()) {
        this._scene.onBeforeRenderObservable.remove(checkCollision); // Arrêter la vérification
        return;
      }

      // Vérifier si le dé entre en collision avec le sol
      if (ground && hitbox.intersectsMesh(ground, false)) {
        console.log("Le dé est entré en contact avec le sol. Suppression...");
        this._diceMesh.dispose(); // Supprimer le mesh du dé
        hitbox.dispose(); // Supprimer le hitbox
        this._scene.onBeforeRenderObservable.remove(checkCollision); // Arrêter la vérification
        // if (onDiceLanded) {
